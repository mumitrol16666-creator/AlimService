...
const home = {
  mount(el) {
    ...
    const steps = [
      ['client', 'message-circle', 'Клиент пишет', 'Из 2ГИС, Instagram или с сайта. Бот отвечает за секунду, на русском или казахском, и собирает заявку: модель, поломка, фото, удобный филиал.', 'Сейчас: ответ по полдня'],
      ['duty', 'headset', 'Дежурный называет цену', 'Мастеру приходит готовая карточка с подсказкой цены. Один тап — клиент получил цену и время записи. Не ответил за 15 минут — заявка уходит запасному, потом владельцу.', 'Сейчас: каждый отвечает со своего телефона'],
      ...
      ['master', 'receipt', 'Квитанция и статусы', 'Клиент получает квитанцию по ссылке: что делают, сколько внёс, сколько осталось, гарантия. О готовности система напишет ему сама.', 'Сейчас: «позвоните, узнайте»'],
    ...
    <div class="home">
      ...
      <h1>От первого сообщения клиента до&nbsp;выданного телефона — в&nbsp;одной системе</h1>
      <p class="lead">Пройдите путь одного клиента за 3 минуты. Каждый шаг можно нажать.</p>
    ...
    icons();
    el.addEventListener('change', e => { if (e.target.id === 'ao') { store.s.alwaysOpen = e.target.checked; store.save(); toast(e.target.checked ? 'Бот и таймеры работают как днём' : 'Вне графика бот предупреждает, что мастер ответит в 10:00', 'clock'); } });
